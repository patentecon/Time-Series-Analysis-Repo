...
make_yearmonth(year = 2023, month = 4:5)
...
x <- seq(as.Date("2023-01-01"), as.Date("2023-04-30"), by = "1 month")
yearmonth(x)
...
We can create a new variable using `mutate()` function.

```{r }
#| echo: true
#| message: true
my_tourism_new <- my_tourism |>
  filter(Region == "Alice Springs")|>
  mutate(Cost = Trips *100)
...
We can select/deselect a variable using `select()` function.

```{r }
#| echo: true
#| message: true
my_tourism_new <- my_tourism_new|>
  filter(Region == "Alice Springs")|>
  mutate(Cost = Trips *100)|>
  select(-Cost)
...
Create new `tsibble` with annual number of trips by state

```{r}
#| echo: true
#| message: true
annual_state_tourism <- my_tourism |>
  group_by(State) |>
  index_by(Year = ~year(.))|>
  summarise(TotalTrips = sum(Trips))

annual_state_tourism 
```
